...
#else
	template <typename Func, typename U>
	using FunctionReturnType = typename std::remove_reference<typename std::remove_cv<typename std::result_of<Func(U)>::
		type>::type>::type;
#endif
} // namespace Catch
...
namespace Catch
{
	...
	struct StringMaker
	{
		...
		static
		typename std::enable_if<Detail::IsStreamInsertable<Fake>::value, std::string>::type
	...
	namespace Detail
	{
		...
		std::string convertUnknownEnumToString(E e)
		{
			return Detail::stringify(static_cast<typename std::underlying_type<E>::type>(e));
		}
...
# endif
	...
	struct StringMaker<char[SZ]>
	{
		static std::string convert(char const* str)
		{
			return Detail::stringify(std::string{str});
		}
	};
	...
	struct StringMaker<signed char[SZ]>
	{
		...
		{
			return Detail::stringify(std::string{reinterpret_cast<char const*>(str)});
		}
	};
...
namespace Catch
{
	...
	{
		return Detail::rangeToString(begin(range), end(range));
	}
	...
	template <typename R>
	struct StringMaker<R, typename std::enable_if<is_range<R>::value && !Detail::IsStreamInsertable<R>::value>::type>
	{
	...
	class BinaryExpr : public ITransientExpression
	{
		...
		void streamReconstructedExpression(std::ostream& os) const override
		{
			formatReconstructedExpression
				(os, Detail::stringify(m_lhs), m_op, Detail::stringify(m_rhs));
		}
...
namespace Catch
{
	...
	public:
		...
		{
			captureValue(index, Detail::stringify(value));
		}
...
namespace Catch
{
	...
	template <>
	struct StringMaker<Detail::Approx>
	{
		static std::string convert(Detail::Approx const& value);
	};
} // end namespace Catch
	...
	namespace Matchers
	{
		namespace Vector
		{
			...
			struct ContainsElementMatcher : MatcherBase<std::vector<T>>
			{
				...
				std::string describe() const override
				{
					return "Contains: " + Detail::stringify(m_comparator);
				}
			...
			struct EqualsMatcher : MatcherBase<std::vector<T>>
			{
				...
				std::string describe() const override
				{
					return "Equals: " + Detail::stringify(m_comparator);
				}
			...
			struct ApproxMatcher : MatcherBase<std::vector<T>>
			{
				...
				std::string describe() const override
				{
					return "is approx: " + Detail::stringify(m_comparator);
				}
				...
				std::vector<T> const& m_comparator;
				mutable Detail::Approx approx = Detail::Approx::custom();
			};
			...
			struct UnorderedEqualsMatcher : MatcherBase<std::vector<T>>
			{
				...
				std::string describe() const override
				{
					return "UnorderedEquals: " + Detail::stringify(m_target);
				}
...
namespace Catch
{
	...
	using StringMatcher = MatcherBase<std::string>;
		...
		public:
				...
				m_predicate(std::forward<P>(pred))
			{
				if (!m_predicate(m_generator.get()))
				{
					...
					{
						throw_exception(GeneratorException("No valid value found in filtered generator"));
					}
				}
			}
				...
				m_chunk_size(size), m_generator(std::move(generator))
			{
				...
				if (m_chunk_size != 0)
				{
					...
					for (size_t i = 1; i < m_chunk_size; ++i)
					{
						if (!m_generator.next())
						{
							throw_exception(GeneratorException("Not enough values to initialize the first chunk"));
						}
						m_chunk.push_back(m_generator.get());
					}
				}
	...
	namespace Generators
	{
		...
		public:
			...
			IteratorGenerator(InputIterator first, InputSentinel last): m_elems(first, last)
			{
				if (m_elems.empty())
				{
					throw_exception(GeneratorException("IteratorGenerator received no valid values"));
				}
			}
